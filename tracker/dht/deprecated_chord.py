...
class BadStateException(Exception):
    '''
    Exception raised when a node enter a wanky state.
    It means the node runs out of succesors, for example.
    '''
    pass
...
def _in_interval(node, node_left, node_right):
    node = node % 2**KEY_SIZE
    node_left = node_left % 2**KEY_SIZE
    node_right = node_right % 2**KEY_SIZE
...
class Node:
    ...
    def __init__(self, ip_address, port, dest_host=None):
        ...
        while 1:
            logging.info("** NODE STATE **\n Succesors: {}\nPredecesor:{}".format(self.succesor, self.predecesor))
            sleep(6)
    ...
    def _get_live_succesor(self, key=0):
        for node in [self.finger[0]] + self.succesor:
            try:
                if node[Node.Id] == self.identifier:
                    return node
                result = request(
                    "chord://%s:%d" % (node[Node.Id], node[Node.Port]),
                    'ping',
                    node[Node.Id]
                )
                self.finger[0] = node
                ...
                logging.debug("Not found {}".format(node))
                pass
        self.lock.release()
        raise BadStateException("No succesor available")

    def _get_live_succesor_deprecated(self):
        blacklist = []
        result = self.node
    ...
    def find_succesor(self, key):
        if self.predecesor and _in_interval(key, self.predecesor[Node.Id], self.identifier):
            return self.node
    ...
    def find_predecessor(self, key):
        succ = self._get_live_succesor()
        node = self.node
        if succ[Node.Id] == self.identifier:
            return self.node
        while not _in_interval(key, node[Node.Id], succ[Node.Id]):
            node = request(
                "chord://%s:%d" % (node[Node.Ip], Node.Port),
    ...
    def deprecated_find_succesor(self, key):
        '''
    ...
    def deprecated_closest_preceding_node(self, key):
        '''
    ...
    def closest_preceding_node(self, key):
        for node in reversed(self.succesor + self.finger):
            if node is not None and _in_interval(node[Node.Id], self.identifier, key):
                try:
                    request(
                        "chord://%s:%d" % (node[Node.Ip], node[Node.Port]),
                        'ping',
    ...
    def stabilize(self):
        # A node x might be our new succesor if:
        # - x = predecessor(succesor(self))
        # - x exists
        # - x is in range(self, succ(self))
        # - if our succesor failed
        succesor = self._get_live_succesor()
        succesor_id = succesor[Node.Id]
        if succesor_id != self.finger[0][Node.Id]:
            self.finger[0] = succesor
        try:
            x = request(
                "chord://%s:%d" % (succesor[Node.Ip], succesor[Node.Port]),
                "get_predecessor",
        ...
        if x is not None and\
                _in_interval(x[Node.Id], self.identifier, succesor[Node.Id]):
            self.finger[0] = x
        succesor = self._get_live_succesor()
        request("chord://%s:%d" % (succesor[Node.Ip], succesor[Node.Port]), 'notify', self.node)
        return True
